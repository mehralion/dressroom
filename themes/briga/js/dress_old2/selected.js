...
function Selected(categoryIn) {
    ...
    this.setApRune = function (valIn) {
        ...
        if(_apInfoRune['select'] > 0) {
            ...
            $.each(_changeStats, function(name, value){
                if(value - _apInfoRune['stat'] >= 0) {
                    _changeStats[name] = value - _apInfoRune['stat'];
                    return false;
                } else {
                    _changeStats[name] = 0;
                    _apInfoRune['stat'] -= value;
                }
            });
        }
        _apInfo = {
    ...
    this.getSetStats = function () {
        ...
        var add = 0;
        if(!_that.isWeapon())
            add = _that.getStatsMf() + art + _apInfo['stat'] - alreadySet;
        else
            add = art - alreadySet;
        return add;
    }
    ...
    this.getSetMF = function() {
        ...
        var add = 0;
        if(!_that.isWeapon())
            add = _apInfo['mf'] + art + _podgonInfo['mf'] - alreadySet;
        else
            add = art - alreadySet;
        return add;
    }
    ...
    this.setItem = function() {
        ...
        $.each(_item['params_give']['bron'], function(name, value){
            if(value > 0 || _that.isArt()) {
                if(value == 0 && _that.getArtBron() == 0)
                    return;
                _item['params_give']['bron'][name] += _that.getApBron() + _that.getBronMf() + _that.getArtBron();
            }
        });
        /* end */
        /* устанавливаем износ */
        ...
        var ap = _that.getSetAp();
        if(ap > 0) {
            _item['params_need']['other']['level'] = apInfo[ap]['level'];
        ...
        _writeSettings();
        var category = container[tab].setItem(_item, _itemCategory, _itemId); // вешаем вещь
     ...
     * @param valueIn
     */
    this.setChooseUsil = function(valueIn) {
        _chooseUsil = valueIn;
    }
    /**
     * Получаем усилене
    ...
    var __construct = function (categoryPassed) {
        _item = $.extend(true, {}, container[tab].getCurrItem(categoryPassed));
