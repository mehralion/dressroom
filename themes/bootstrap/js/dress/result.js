...
function Result() {
    ...
    this.drawResult = function() {
        ...
        $.each(container, function(tabCont, itemContainer){
            ...
            _id = tabCont;
            _Tab = $('<div>', {'class':'item_' + tabCont, 'style':'display:inline-block'}).appendTo('.tabResult .c2');
            var _HeadCont = $('<ul>', {'style':'height: 15px;margin-bottom: 0px;'}).appendTo(_Tab);
    ...
    this.drawUron = function() {
        var _UronCont = $('<ul>', {}).appendTo(_Tab);
        var maxU = 0;
        var minU = 0;
        var vladCount = 0;
    ...
    this.drawStatsAndVlad = function (categoryIn) {
        var _StatsCont = $('<ul>', {}).appendTo(_Tab);
        var _statsAll = 0;
        ...
        $('<li>', {text:_statsAll, value:_statsAll}).appendTo(_StatsCont);
        $('<div>', {'class':'separator'}).appendTo(_Tab);
    }
    this.drawTableParams = function (categoryIn) {
        var _MfCont = $('<ul>', {'class':'table_mf'}).appendTo(_Tab);
        var haveParams = _container.getGroupParam(categoryIn, 'have');
        $.each(haveParams, function (statName, statValue) {
            var pers = '';
            if (categoryIn == 'mf') {
